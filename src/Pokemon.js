...
function Pokemon({ userdata }) {
  ...
  function ClickToReview() {
    addReview(userdata['username'], id, RatingValue, ReviewTitle.current.value, ReviewBody.current.value).then((data) => {
      setUserReview(data["reviews"][0]);
    });
  }
  ...
  return (
    <div>
      ...
      <div><img src={PokemonInfo['pic']} width={150} height={150} /></div>
      {(TotalReview === null) ?
        ...
        : (<div>
          ...
          {TotalReview.map((review) => (
            <li key={review.username}>
              {review.name}/{review.username}/<Rating name="read-only" value={review.rating} readOnly />/{review.title}/{review.body}
            </li>
          ))}
        </div>
        )
        ...
        (UserReview === null) ? (
          <details>
            ...
            <form>
              ...
              <button type="button" onClick={ClickToReview}>Submit Review</button>
            </form>
          </details>
        ) : (
          <div>
            <hr />
            {userdata.name}/{UserReview.username}/<Rating name="read-only" value={`${UserReview.rating}`} readOnly />/{UserReview.title}/{UserReview.body}
          </div>
        )
      }
    </div >
